...
@export var front_art: Texture2D = preload("res://Assets/Textures/CardTextures/CardFrontTexture.png"):
	set(value):
		front_art = value
		_refresh()
@export var back_art: Texture2D = preload("res://Assets/Textures/CardTextures/CardBackTexture.png"):
	set(value):
		back_art = value
		_refresh()
...
@export_range(1, 120, 1) var simulated_camera_fov: float = 33:
	set(value):
		simulated_camera_fov = value
		_refresh()
@export_range(-360, 360, 1) var rotation_y: float = 180.0:
	set(value):
		rotation_y = value
		_refresh()
