...
func set_gap()->void:
	...
	if cards.size() > 0:
		...
		var top_and_bottom_margins : float = card_height
		if (total_stack_height + top_and_bottom_margins) > height:
			var overflow_percentage : float = (height - top_and_bottom_margins) / total_stack_height
